...
# Fast ASCII lookup for AA->index mapping (for vectorized DPC)
AA_TO_IDX = {a: i for i, a in enumerate(std)}
_LUT = np.full(256, -1, dtype=np.int16)
for _i, _a in enumerate(std):
    _LUT[ord(_a)] = _i

def _dpc_vector_for_sequence(seq, q):
    s = str(seq).strip().upper()
    L = len(s)
    denom = max(1, L - q)
    if L <= q:
        return np.zeros(400, dtype=np.float32)
    # Vectorized map: ASCII bytes -> indices via LUT
    try:
        bs = np.frombuffer(s.encode('ascii'), dtype=np.uint8)
    except Exception:
        # Fallback if non-ascii slips in
        bs = np.fromiter((ord(ch) for ch in s), dtype=np.uint16, count=L).astype(np.uint8)
    idx = _LUT[bs].astype(np.int32)
    first = idx[:-q]
    second = idx[q:]
    # Count only valid AA pairs; denominator remains (L - q) to match original
    mask = (first >= 0) & (second >= 0)
    if not np.any(mask):
        return np.zeros(400, dtype=np.float32)
    pairs = (first[mask] * 20 + second[mask]).astype(np.int32)
    counts = np.bincount(pairs, minlength=400).astype(np.float32)
    return (counts / float(denom)) * 100.0
...
def dpc_comp(file,q,out):
    filename, file_extension = os.path.splitext(file)
    print(f"[DPC{q}] Loading input '{file}'...", file=sys.stderr, flush=True)
    f = open(out, 'w')
    sys.stdout = f
    df = pd.read_csv(file, header = None)
    ...
    if workers and workers > 1:
        print(f"[DPC{q}] Using {workers} worker threads...", file=sys.stderr, flush=True)
        results = [None] * len(seqs)
        with ThreadPoolExecutor(max_workers=workers) as ex:
            fut_to_idx = {ex.submit(_dpc_vector_for_sequence, s, q): i for i, s in enumerate(seqs)}
            for fut in tqdm(as_completed(fut_to_idx), total=len(seqs), desc=f"DPC{q}: sequences", unit="seq"):
                i = fut_to_idx[fut]
        ...
        for s in tqdm(seqs, desc=f"DPC{q}: sequences", unit="seq"):
            vec = _dpc_vector_for_sequence(s, q)
            for val in vec:
                print("%.2f" % val, end=',')
            print("")
    f.truncate()
def dpc_split(file,q,n,out):
    ...
    print(f"[DPC{q}] Preparing {n} splits per sequence from '{file}'...", file=sys.stderr, flush=True)
    df1 = pd.read_csv(file, header = None)
    df2 = pd.DataFrame(df1[0].str.upper())
    k1 = []
    # Build splits with progress over input sequences
    for w in tqdm(range(0, len(df2)), desc="split: sequences", unit="seq"):
        s = 0
        k2 = []
        r = 0
        if len(df2[0][w])%n == 0:
    ...
    print("")
    # Compute vectors for each split (ordered) with optional threading
    workers = args.workers if hasattr(args, 'workers') and args.workers else (cpu_count() or 1)
    splits = list(k1)
    if workers and workers > 1:
        print(f"[DPC{q}] Using {workers} worker threads for splits...", file=sys.stderr, flush=True)
        vecs = [None] * len(splits)
        with ThreadPoolExecutor(max_workers=workers) as ex:
            fut_to_idx = {ex.submit(_dpc_vector_for_sequence, s, q): i for i, s in enumerate(splits)}
            for fut in tqdm(as_completed(fut_to_idx), total=len(splits), desc=f"DPC{q}: splits", unit="split"):
                i = fut_to_idx[fut]
                vecs[i] = fut.result()
    else:
        vecs = []
        for s in tqdm(splits, desc=f"DPC{q}: splits", unit="split"):
            vecs.append(_dpc_vector_for_sequence(s, q))
    # Write one row per original sequence group (n splits per row)
    for i in tqdm(range(0, len(vecs), n), desc=f"DPC{q}: groups", unit="grp"):
        group = vecs[i:i+n]
        for g in group:
            for val in g:
                print("%.2f" % val, end=',')
